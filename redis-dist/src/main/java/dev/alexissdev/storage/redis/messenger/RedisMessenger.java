...
import dev.alexissdev.storage.redis.messenger.pubsub.RedisSubChannelPubsub;
...
/**
 * The RedisMessenger class provides an implementation of the {@link Messenger} interface
 * to enable inter-server or inter-component communication using Redis as the underlying
 * messaging platform. It supports pub-sub mechanisms for listening and publishing to channels.
 */
...
public class RedisMessenger
    ...
    public RedisMessenger(
        ...
        executor.execute(() -> listenerConnection.subscribe(pubSub, parentChannel));
    }
    ...
    public <T> Channel<T> getChannel(String name, TypeToken<T> type) {
        ...
        if (channel == null) {
            channel = new RedisChannel<>(parentChannel, serverId, name, type, this, messengerPool, gson);
